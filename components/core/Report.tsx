...
export default function Report({ onClose }: ReportProps) {
  ...
  const [from, setFrom] = useState('TechTitanByte');
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setError(null);
    setFrom('TechTitanByte')
    setBucketKey('a9f00191-5799-4028-9ad4-69ed6d1228be')
    ...
    try {
      const response = await fetch('https://unilibs.vercel.app/api/report/log&upload', {
        ...
        body: JSON.stringify({ title, problem, from, bucketKey }),
      });
